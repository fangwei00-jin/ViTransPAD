...
def main_worker():
    ...
    model = net.InpaintGenerator(1,[0,1,2,3]).to(device)
    ...
    for _ in range(len(frames)):
        m = Image.fromarray(np.zeros((h, w)).astype(np.uint8))
    ...
    for f in range(0, video_length, neighbor_stride):
        ...
        with torch.no_grad():
            ...
            pred_imgs = torch.tanh(model.decoder(
    ...
    writer = cv2.VideoWriter(f"{vidname}_result.mp4", cv2.VideoWriter_fourcc(*"mp4v"), default_fps, (w, h))
    for f in range(video_length):
        # comp = np.array(comp_frames[f]).astype(
        #     np.uint8)*binary_masks[f] + frames[f] * (1-binary_masks[f])
        comp = np.array(comp_frames[f]).astype(np.uint8)
        writer.write(cv2.cvtColor(np.array(comp).astype(np.uint8), cv2.COLOR_BGR2RGB))
    writer.release()
    print('Finish in {}'.format(f"{vidname}_result.mp4"))
