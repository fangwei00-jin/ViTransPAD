...
class AblationCAM(BaseCAM):
    ...
    def get_cam_weights(self,
                        ...
                        grads):
        with torch.no_grad():
            outputs = self.model(input_tensor, mask_tensor).cpu().numpy()
            original_scores = []
        ...
        with torch.no_grad():
            ...
            for tensor, mask_tensor, category in zip(input_tensor, mask_tensor, target_category):
                batch_tensor = tensor.repeat(BATCH_SIZE, 1, 1, 1)
                batch_tensor_mask = mask_tensor.repeat(BATCH_SIZE, 1, 1, 1)
                for i in tqdm.tqdm(range(0, number_of_channels, BATCH_SIZE)):
                    ...
                    if i + BATCH_SIZE > number_of_channels:
                        ...
                        batch_tensor_mask = batch_tensor_mask[:keep]
                        ablation_layer.indices = ablation_layer.indices[:keep]
                    score = self.model(batch_tensor, batch_tensor_mask)[:, category].cpu().numpy()
