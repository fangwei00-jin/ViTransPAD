...
class Evaluate():
    ...
    def infer(self, faceframes, masks, device):
        ...
        for f in range(0, video_length, self.neighbor_stride):
            ...
            with torch.no_grad():
                ...
                pred_imgs, y_dec = self.model.infer(faces_tensor_mask, mask_tensors_clip)
        ...
        return pred_depth_maps, y_dec_video
    ...
    def get_depth_map_score(self, df_video, videos, device, ep, threshold=-1):
        ...
        for i, video in enumerate(videos):
            ...
            # bp()
            pred_depth_maps, y_dec_video = self.infer(faces, masks, device)
            scores_vid_dec = self.frames_class_scores(y_dec_video, spoofing_label)
            score_list_dec += scores_vid_dec
        ...
        return map_score_list, spoof_list, score_list_enc, score_list_dec
    ...
    def evaluate(self, ep, ckpt):
        ...
        map_score_list, spoof_list, _, _ = self.get_depth_map_score(self.val_df_video, val_videos, self.device, ep)
        #bp()
        ...
        ### test ###
        map_score_list, spoof_list, scores_vid_enc, scores_vid_dec = self.get_depth_map_score(self.df_video, videos, self.device, ep, threshold)

        ACC, APCER, BPCER, ACER = self.performance(map_score_list, spoof_list, threshold)
        ACC_cls_enc, APCER_cls_enc, BPCER_cls_enc, ACER_cls_enc = self.performance_cls(scores_vid_enc, spoof_list)
        ACC_cls_dec, APCER_cls_dec, BPCER_cls_dec, ACER_cls_dec = self.performance_cls(scores_vid_dec, spoof_list)
