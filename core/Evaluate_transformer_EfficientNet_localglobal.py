...
class Evaluate():
    ...
    def infer(self, faceframes, masks, device):
        ...
        for f in range(0, video_length, self.neighbor_stride):  ## overlaping neighbor_stride frames
        # for f in range(0, video_length, 2*self.neighbor_stride+1):  ## no overlapping
        ...
        #for f in range(0, video_length, 1):  ## overlap  2$neighbor_stride-2 frames
            ...
            with torch.no_grad():
                ...
                faces_tensors_clip = faces_tensors[0, infer_frames_ids+ref_ids, :, :, :]
                mask_tensors_clip = mask_tensors[0, infer_frames_ids+ref_ids, :, :, :]
                faces_tensors_clip, mask_tensors_clip = faces_tensors_clip.to(device), mask_tensors_clip.to(device)
                faces_tensor_mask = (faces_tensors_clip * (1 - mask_tensors_clip).float()).view(1, len(infer_frames_ids+ref_ids), 3, self.h, self.w)
                # feats = self.model.encoder(
                #     (feats_tensors_clip * (1 - mask_tensors_clip).float()).view(len(neighbor_ids+ref_ids), 3, self.h, self.w))
                #feats = self.model.encoder.extract_features()

                # ## predict enc features with encoder feats calculated in one time
                # y_enc_logits = self.model.infer(
                #     feats[0, neighbor_ids + ref_ids, :, :, :], mask_tensors[0, neighbor_ids + ref_ids, :, :, :])
                ## predict enc features
                y_enc_logits = self.model.infer(faces_tensor_mask, mask_tensors_clip)

                # pred_imgs = torch.tanh(self.model.decoder(
                #     pred_feat[:len(neighbor_ids), :, :, :])).detach()
                y_enc_logits = y_enc_logits[:len(neighbor_ids)]
                # pred_imgs = (pred_imgs + 1) / 2
                # pred_imgs = pred_imgs.cpu().permute(0, 2, 3, 1).numpy() * 255
                for i in range(len(neighbor_ids)):
                    idx = neighbor_ids[i]
                    infer_frames_ids_list.append(idx)
                    y_logit = y_enc_logits[i]
                    if y_enc_video[idx] is None:
                        y_enc_video[idx] = y_logit
                    else:
                        y_enc_video[idx] = y_enc_video[idx] * 0.5 + y_logit * 0.5
        y_logits = []
        for y in y_enc_video:
            if y is not None:
                y_logits.append(y)

        # infer_frames_ids = list(range(0, video_length, math.ceil(video_length/self.ref_length)))
        # faces_tensors_clip = faces_tensors[0, infer_frames_ids, :, :, :]
        # mask_tensors_clip = mask_tensors[0, infer_frames_ids, :, :, :]
        # faces_tensors_clip, mask_tensors_clip = faces_tensors_clip.to(device), mask_tensors_clip.to(device)
        # faces_tensor_mask = (faces_tensors_clip * (1 - mask_tensors_clip).float()).view(1, len(infer_frames_ids), 3,
        #                                                                                 self.h, self.w)
        # y_logits = self.model.infer(faces_tensor_mask, mask_tensors_clip)

        infer_frames_ids_list = list(set(infer_frames_ids_list))
        # return pred_depth_maps
        return y_logits, infer_frames_ids_list
    ...
    def performance (self, scores, labels, threshold=0.5):
        idx_type1 = [i for i, s in enumerate(scores) if s < threshold and labels[i] == 1] ## false negative
        idx_type2 = [i for i, s in enumerate(scores) if s < threshold and labels[i] == 0] ## false positive
    ...
    def get_depth_map_score(self, df_video, videos, device, ep, threshold=-1):
        ...
        for i, video in enumerate(videos):
            ...
            y_transformer, infer_frames_ids = self.infer(faces, masks, device)
            ...
            spoof_list += [spoofing_label] * len(y_transformer)

            false_live = 0
            false_attack = 0
            for k, score in enumerate(scores_vid):
                frame_index = infer_frames_ids[k]
                image_name = str.split(images_path[frame_index], '/')[-1]
