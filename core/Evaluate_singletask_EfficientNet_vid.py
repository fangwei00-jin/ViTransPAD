...
class Evaluate():
    ...
    def read_frame_from_video(self, df_video, vidname):
        ...
        for idx in range(video_length)[::self.test_frames_sample_interval]:
            ...
            #image_path= '/data/zming/datasets/Anti-spoof/SELFCOLLECT_Data/AriadNext/exemple_video_attaque_avec_flash/spoof/001/0131.jpg'
            #print(image_path)
            bbox = bboxes[idx]
            # print(image_path)
            xx = str.split(bbox, " ")
    ...
    def infer(self, faceframes, masks, device):
        ...
        #     feats_all = feats.view(1, video_length, c, feat_h, feat_w)
        ...
        for f in range(0, video_length, self.neighbor_stride):
            ...
            with torch.no_grad():
                ...
                pred_imgs = self.model.infer(faces_tensor_mask, mask_tensors_clip).detach()
                # pred_imgs = torch.tanh(self.model.decoder(pred_feat[:len(neighbor_ids), :, :, :])).detach()
    ...
    def get_depth_map_score(self, df_video, videos, device, ep, threshold=-1):
        ...
        for i, video in enumerate(videos):
            ...
            # ## debug ###
            # # if threshold != -1:
            # #     video = '032-2-1-2-2''1-3-46-1' ## false positive case
            # video = '032-2-1-2-2'
            # ### debug ###
            ...
            if threshold != -1:
                ...
                for k, score in enumerate(score_list):
                    ...
                    if score > threshold and spoofing_label == 0:
                        ...
                        cv2.imwrite(os.path.join(APCE_dir, "%s_%04d_map_gt.jpg"%(video, face_frm)), map_gt_cv)
                        cv2.imwrite(os.path.join(APCE_dir, "%s_%04d_map_pre.jpg"%(video, face_frm)), map_cv)
                        false_live += 1
                    elif score < threshold and spoofing_label == 1:
                        cv2.imwrite(os.path.join(BPCE_dir, "%s_%04d_face.jpg"%(video, face_frm)), face_cv)
                        cv2.imwrite(os.path.join(BPCE_dir, "%s_%04d_map_gt.jpg"%(video, face_frm)), map_gt_cv)
                        cv2.imwrite(os.path.join(BPCE_dir, "%s_%04d_map_pre.jpg"%(video, face_frm)), map_cv)
                        false_attack += 1
                    else:
                ...
                pred_frames = [1 if score > threshold else 0 for score in score_list]
        ...
        if threshold == -1:
            return map_score_list, spoof_list
        else:
    ...
    def evaluate(self, ep, ckpt):
        ...
        ### test ###
        map_score_list, spoof_list = self.get_depth_map_score(self.df_video, videos, self.device, ep, threshold)
