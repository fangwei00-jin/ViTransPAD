...
class Dataset(torch.utils.data.Dataset):
    ...
    #     all_frames = os.listdir(os.path.join(self.args['data_root'], self.args['name'], 'train', 'JPEGImages', video_name))
    #     all_masks = create_random_shape_with_random_motion(len(all_frames), imageHeight=self.h, imageWidth=self.w)
    #     frames = []
    #     masks = []
    #     for idx in range(len(all_frames))[:20]:
    #         # img = cv2.imread(os.path.join(self.args['data_root'], self.args['name'], 'train', 'JPEGImages', video_name, all_frames[idx]))
    #         # if img is None:
    #         #     print(os.path.join(self.args['data_root'], self.args['name'], 'train', 'JPEGImages', video_name, all_frames[idx]))
    #         # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    #         # img = cv2.resize(img, self.size, interpolation = cv2.INTER_AREA)
    #         img = Image.open(os.path.join(self.args['data_root'], self.args['name'], 'train', 'JPEGImages', video_name, all_frames[idx])).convert('RGB')
    ...
    def load_item(self, index):
        ...
        for idx in ref_index:
            ...
            img = Image.open(os.path.join(self.args['data_root'], self.args['name'], 'train_all_frames', 'JPEGImages', video_name, all_frames[idx])).convert('RGB')
