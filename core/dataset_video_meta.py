...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, args: dict, split='train', debug=False):
        ...
        self.modalities = []
        self.split = split
        if args['train']['meta_learning'] != '':
            self.isMeta = True
            if self.split == 'train':
                meta_datasets_files = args['train']['modality'][0]
                meta_datasets = str.split(meta_datasets_files, ';')
                for meta_traintest_i in meta_datasets:
                    meta_train_i = str.split(meta_traintest_i, ',')[0]
                    meta_test_i = str.split(meta_traintest_i, ',')[1]
                    self.video_files.append(os.path.join(args['train']['data_root'], meta_train_i))
                    self.video_files.append(os.path.join(args['train']['data_root'], meta_test_i))
            ...
            raise valueError("Please input the Meta-learning strategy, e.g. MAML !")
        ...
        for video_file in self.video_files:
            df_video = json.load(open(video_file))
            self.df_videos_list.append(df_video)

        self.video_names_list = []
        for df_video in self.df_videos_list:
            video_names = list(df_video.keys())
            self.video_names_list.append(video_names)
    ...
    def __len__(self):
        ...
        if self.split=='train':
            ...
            #return  1## debug
            return self.args['train']['trainer']['update_step']  ## debug
        else:
            return int(len(self.df_videos_list[0])) ## total videos for test = batch * batch_size
            #return 10  ## debug
    ...
    def __getitem__(self, index):
        items_list = []
        for i in range(len(self.df_videos_list)):
            try:
                df_videos = self.df_videos_list[i]
                video_names = self.video_names_list[i]
                vidname = video_names[index]
                item = self.load_item(df_videos, vidname)
                # ## debug
                # print(item["video"])
            except:
                print('Loading error in video {}\n'.format(vidname))
                vidname = video_names[0]
                item = self.load_item(df_videos, vidname)
                # ## debug
                # print(item["video"])
    ...
    def load_item(self, df_video, vidname):
        item = {}
        if debug:
            video = "129-2-2-2-1" ##debug
            ##video = "real_client103_session01_webcam_authenticate_controlled_1_2" ##debug
        else:
            video = vidname #self.video_names[index]
        all_frames = df_video[video]['frames'] ##RGB images
        all_masks = []
        ...
        if self.classes == 2:
            spoofing_label = df_video[video]['spoofing_label'] ## 1 Bonafide | 0 Attack
        else:
        ...
        bboxes = list(df_video[video]['bboxes'])
