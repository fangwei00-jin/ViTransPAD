...
class Trainer():
    ...
    def _train_epoch(self, pbar):
        ...
        for batch_idx, batch  in enumerate(self.train_loader):
            ...
            pred_depth_maps, y_dec, y_enc = self.netG(masked_frame, masks)
            ...
            gen_loss = 0.1*loss_softmax_depth + 0.1*loss_softmax_enc
            dis_loss = 0
            ...
            real_vid_feat = self.netD(depth_maps_resize)
            #fake_vid_feat = self.netD(comp_img.detach())
            fake_vid_feat = self.netD(pred_depth_maps.detach())
            ## first True/Fake means inputing real/generated fake image feature, second True/False means for training Discriminator or not
            ...
            ## gan_loss is the loss of the similarity of the generated depth map to the real depth map
            ## i.e. the discriminator's loss if we treat the generated depth map as the real one
            ...
            #     if p.requires_grad:
            #         print(name)
            # for name, p in self.netD.named_parameters():
            #     if p.requires_grad:
            #         print(name)
            ...
            if self.config['global_rank'] == 0:
                ...
                if self.iteration % write_summary_freq == 0:
                    ...
                    pred_map_batch_show = np.zeros((n, pred_depth_maps.shape[2], pred_depth_maps.shape[3], pred_depth_maps.shape[1]),
                        dtype=np.uint8)
                    pred_depth_maps_detach = pred_depth_maps.detach()
                    pred_depth_maps_detach = (pred_depth_maps_detach + 1) / 2
                    pred_depth_maps_detach = pred_depth_maps_detach.cpu().permute(0, 2, 3, 1).numpy() * 255
                ...
                if self.config['global_rank'] == 0:
                    ...
                    print('Train Epoch: {}/{} [ {}/{} batch)] time:{:.2f} \tdepth_map_gen_loss: {:.6f}, gan_loss: {:.6f}  dis_loss: {:.6f} loss_softmax_depth : {:.6f} loss_softmax_enc : {:.6f}'.format(
                            int(self.epoch), int(self.train_args['iterations']/len(self.train_loader)), (batch_idx), len(self.train_loader),
                             abs(tt[-1] - tt[-2]), valid_loss.item(), gan_loss.item(), dis_loss.item(), loss_softmax_depth.item(), loss_softmax_enc.item()))
