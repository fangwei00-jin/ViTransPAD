...
class GroupRandomHorizontalFlip(object):
    ...
    def __call__(self, ListImages, is_flow=False):
        ListImagesFlip = []

        v = random.random()
        if v < 0.5:
            for img_group in ListImages:
                img_group_flip = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in img_group]
                if self.is_flow:
                    for i in range(0, len(img_group_flip), 2):
                        # invert flow pixel values when flipping
                        img_group_flip[i] = ImageOps.invert(img_group_flip[i])
                ListImagesFlip.append(img_group_flip)
            return ListImagesFlip
        else:
            return ListImages
