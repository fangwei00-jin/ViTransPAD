...
class Evaluate():
    def __init__(self, config, split, model=None):
        self.log_dir = config['test']['save_dir']
        ...
        for f in range(0, video_length, self.neighbor_stride):
            ...
            with torch.no_grad():
                ...
                feats = self.model.encoder(
                    (feats_tensors_clip * (1 - mask_tensors_clip).float()).view(len(neighbor_ids + ref_ids), 3, self.h,
                                                                                self.w))
                ...
                pred_feat = self.model.infer(feats, mask_tensors_clip)
