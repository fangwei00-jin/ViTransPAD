...
class Trainer():
    ...
    # load netG and netD
    def load(self):
        ## load the last pretrained model in the same directory to recover the training
        ...
        if latest_epoch is not None:
            ...
            ## load the specific pretrained model for fine-tuning
            if len(self.config['pretrained_model']) > 0:
                pretrained_model_path = self.config['pretrained_model']
                print('Loading pretrained model: %s' % pretrained_model_path)
                data = torch.load(pretrained_model_path, map_location=self.config['device'])
                self.netG.load_state_dict(data['netG'], strict=False)
            else:
                if self.config['global_rank'] == 0:
    ...
    def train(self):
        ...
        ACER_all = []
        threshold_all = []
        loss_file = open(os.path.join(self.config['save_dir'], 'loss_weight.txt'), 'w')
        loss_file.write('loss_transformer_softmax, loss_depth_map_softmax, loss_generator, alpha1, alpha2, alpha3\n')
        while True:
            self.epoch += 1
            if self.config['distributed']:
                self.train_sampler.set_epoch(self.epoch)

            self._train_epoch(pbar, loss_file)
    ...
    def _train_epoch(self, pbar, loss_file):
        tt = [0.0, 0.0]
        ...
        for batch_idx, batch  in enumerate(self.train_loader):
            ...
            y_enc, pred_depth_maps, y_dec = self.netG(masked_frame, masks)
            ...
            #bp() ## debug
            ## transformer y_enc, generator depth map y_dec classification
            ...
            ## Resize the groud truth of depth map and mask to the size of the generated depth map to calculate the loss
            b, c, h, w = pred_depth_maps.size()
            depth_maps_resize = F.interpolate(depth_maps, size=h)
            masks_resize = F.interpolate(masks, size=h)
            valid_loss = self.l1_loss(pred_depth_maps*(1-masks_resize), depth_maps_resize*(1-masks_resize))
            valid_loss = valid_loss / torch.mean(1-masks_resize) * self.config['losses']['valid_weight']

            total_loss = loss_softmax_enc+loss_softmax_depth+valid_loss
            alpha1= loss_softmax_enc/total_loss
            alpha2= loss_softmax_depth/total_loss
            alpha3= valid_loss/total_loss



            gen_loss = alpha1 * loss_softmax_enc + alpha2 * loss_softmax_depth + alpha3 * valid_loss

            self.add_summary(
                self.gen_writer, 'loss/generated_depth_map_l1_loss', valid_loss.item())
            ...
            # # ## Print the number parameters
            # sum = 0
            # for p in self.netG.parameters():
            #     sum += p.numel
            # print("Model has %d parameters."%sum)
            ...
            if self.config['global_rank'] == 0:
                ...
                if self.iteration % write_summary_freq == 0:
                    ...
                    pred_map_batch_show = np.zeros((n, pred_depth_maps.shape[2], pred_depth_maps.shape[3], pred_depth_maps.shape[1]), dtype=np.uint8)
                    pred_depth_maps_detach = pred_depth_maps.detach()
                    pred_depth_maps_detach = (pred_depth_maps_detach + 1) / 2
                    pred_depth_maps_detach = pred_depth_maps_detach.cpu().permute(0, 2, 3, 1).numpy() * 255
                ...
                if self.config['global_rank'] == 0:
                    ...
                    print(
                        'Train Epoch: {}/{} [ {}/{} batch)] time:{:.2f} \n loss_softmax_transformer: {:.6f} loss_softmax_depth: {:.6f} depth_map_gen_loss: {:.6f} \n alpha1: {:.2f} alpha2: {:.2f} alpha3: {:.2f}'.format(
                            int(self.epoch), int(self.train_args['iterations'] / len(self.train_loader)), (batch_idx),
                            len(self.train_loader), abs(tt[-1] - tt[-2]), loss_softmax_enc.item(),
                            loss_softmax_depth.item(),  valid_loss.item(), alpha1, alpha2, alpha3))
                    ...
                    ## write loss to loss.txt
                    loss_file.write('%f %f %f %.2f %.2f %.2f\n'%(loss_softmax_enc.item(), loss_softmax_depth.item(),  valid_loss.item(), alpha1, alpha2, alpha3))
