...
class Trainer():
    ...
    def _train_epoch(self, pbar):
        ...
        for batch_idx, batch  in enumerate(self.train_loader):
            ...
            gen_loss = 0.1*loss_softmax_enc+0.1*loss_softmax_depth
            dis_loss = 0
            ...
            if self.config['global_rank'] == 0:
                ...
                if self.config['global_rank'] == 0:
                    ...
                    print(
                        'Train Epoch: {}/{} [ {}/{} batch)] time:{:.2f}   loss_softmax_transformer: {:.6f} depth_map_gen_loss: {:.6f} loss_softmax_depth: {:.6f}'.format(
                            int(self.epoch), int(self.train_args['iterations'] / len(self.train_loader)), (batch_idx),
                            len(self.train_loader),
                            abs(tt[-1] - tt[-2]), loss_softmax_enc.item(), valid_loss.item(), loss_softmax_depth.item()))
