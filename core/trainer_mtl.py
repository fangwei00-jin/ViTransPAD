...
class Trainer():
    ...
    def _train_epoch(self, pbar):
        ...
        for batch_idx, batch  in enumerate(self.train_loader):
            ...
            ########################### Toatal GAN loss ######################################
            ### Generator loss gen_loss: weight1*Transformer BCE + weight2*depth map BCE + weight3*generator image l1 distance + weight4*generator adversarial loss ###
            ### Discriminator loss dis_loss: dis_loss += (dis_real_loss + dis_fake_loss) / 2
            ##################################################################################
            #bp() ## debug
            gen_loss = 0
            dis_loss = 0

            #gen_loss += 0.01*loss_softmax_enc + 0.01*loss_softmax_depth
            ...
            if self.config['global_rank'] == 0:
                ...
                if self.config['global_rank'] == 0:
                    ...
                    print('Train Epoch: {}/{} [ {}/{} batch)] time:{:.2f} \tgen_loss: {:.6f}  depth_map_gen_loss: {:.6f} depth_map_softmax_loss: {:.6f} tansformer_softmax_loss: {:.6f} dis_loss: {:.6f}'.format(
                            int(self.epoch), int(self.train_args['iterations']/len(self.train_loader)), (batch_idx), len(self.train_loader),
                             abs(tt[-1] - tt[-2]), gen_loss.item(), valid_loss.item(), loss_softmax_depth.item(), loss_softmax_enc.item(), dis_loss.item()))
