...
class InpaintGenerator(BaseNetwork):
    def __init__(self, Transformer_layers, Transformer_heads, channel, patchsize, backbone, init_weights=True):
        ...
        if self.backbone != "Conv":
            if self.backbone == "ResNet50":
            ...
            elif self.backbone == "Vit_scratch":
                ...
                ## Classification vit : Vit from timm, base vit: heads=12, depth=8
                NUM_FINETUNE_CLASSES = 2 ## attack or live
                model_backbone = timm.create_model('vit_base_patch16_224', pretrained=False, num_classes=NUM_FINETUNE_CLASSES) ## vit from timm: scratch
                
            elif self.backbone == "Vit_pretrained":
                # ## Encoder Backbone : Vit from timm, base vit: heads=12, depth=8
                # #model_backbone = timm.create_model('vit_base_patch16_224', pretrained=True) ## vit from timm: pretrained with imagenet

                ## Classification vit : Vit from timm, base vit: heads=12, depth=8
                NUM_FINETUNE_CLASSES = 2 ## attack or live
                model_backbone = timm.create_model('vit_base_patch16_224', pretrained=True,num_classes=NUM_FINETUNE_CLASSES) ## vit from timm: pretrained with imagenet
    ...
    def forward(self, masked_frames, masks):
        ...
        if self.backbone == 'Conv' or self.backbone == 'ResNet50' or self.backbone == 'ResNext50':
            ...
            # classfication logits from backbone Vit pretrained
            logits_cls = self.encoder(masked_frames.view(b * t, c, height, width)) ## vit embeddings is the cls_token
        else:
